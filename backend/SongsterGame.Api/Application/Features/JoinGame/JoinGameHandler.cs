...
public class JoinGameHandler(IGameService gameService) : SyncRequestHandler<JoinGameCommand, Result<JoinGameResponse>>
{
    protected override Result<JoinGameResponse> HandleCommand(JoinGameCommand request)
    {
        ...
        if (game is null)
            return Result.Failure<JoinGameResponse>(Error.NotFound($"Game with code '{request.GameCode}' not found."));
        
        // 3. Validate game state
        if (game.State != GameState.Lobby)
            return Result.Failure<JoinGameResponse>(Error.BusinessRule("Cannot join a game that has already started."));
        
        // 4. Validate max players
        if (game.Players.Count >= Game.MaxPlayers)
            return Result.Failure<JoinGameResponse>(Error.BusinessRule($"Game is full. Maximum {Game.MaxPlayers} players allowed."));
        ...
        if (game.Players.Any(p => p.Nickname.Equals(request.Nickname, StringComparison.OrdinalIgnoreCase)))
            return Result.Failure<JoinGameResponse>(Error.Conflict($"A player with nickname '{request.Nickname}' already exists in this game."));
        
        // 6. Use existing GameService to join (will be migrated to repository pattern later)
        var success = gameService.JoinGame(gameCodeResult.Value, connectionIdResult.Value, nicknameResult.Value);

        if (!success)
            return Result.Failure<JoinGameResponse>(Error.BusinessRule("Failed to join game due to an unexpected error."));
        
        // 7. Get updated game and map to DTO
        var updatedGame = gameService.GetGame(gameCodeResult.Value);
        if (updatedGame is null)
            return Result.Failure<JoinGameResponse>(Error.BusinessRule("Game disappeared after join operation."));
        
        var response = new JoinGameResponse(updatedGame.GameCode, updatedGame.Players.Select(p => new PlayerDto(p.Nickname, p.IsHost)).ToList());
