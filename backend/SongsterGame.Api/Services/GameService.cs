...
public class GameService(ISpotifyService spotifyService) : IGameService
{
    ...
    public Game? GetGame(string gameCode) => _currentGame?.GameCode == gameCode ? _currentGame : null;

    public Game? GetCurrentGame() => _currentGame;

    public bool JoinGame(string gameCode, string connectionId, string nickname)
    {
        var game = GetGame(gameCode);
        if (game is null || game.State != GameState.Lobby || game.Players.Count >= Game.MaxPlayers)
            return false;
    ...
    {
        var game = GetGame(gameCode);
        if (game is null || game.State != GameState.Lobby || game.Players.Count < 2)
            return false;
    ...
    {
        var game = GetGame(gameCode);
        if (game is null || game.MusicDeck.Count == 0)
            return;
    ...
    {
        var game = GetGame(gameCode);
        if (game is null || game.CurrentCard is null || game.State != GameState.Playing)
            return false;
        
        var player = game.Players.FirstOrDefault(p => p.ConnectionId == connectionId);
        if (player is null || game.CurrentPlayer?.ConnectionId != connectionId)
