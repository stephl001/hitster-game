...
/// </summary>
public sealed class CreateGameHandler(IGameService gameService)
    : SyncRequestHandler<CreateGameCommand, Result<CreateGameResponse>>
{
    protected override Result<CreateGameResponse> HandleCommand(CreateGameCommand request)
    {
        ...
        var game = gameService.CreateGame(connectionIdResult.Value, nicknameResult.Value);

        if (game is null)
            return Result.Failure<CreateGameResponse>(Error.Conflict("A game already exists. Only one game at a time is supported."));
        
        // 3. Map to DTO
        var response = new CreateGameResponse(game.GameCode, game.Players.Select(p => new PlayerDto(p.Nickname, p.IsHost)).ToList());
