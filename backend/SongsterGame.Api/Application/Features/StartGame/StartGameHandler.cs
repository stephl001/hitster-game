    ...
    : SyncRequestHandler<StartGameCommand, Result<StartGameResponse>>
{
    protected override Result<StartGameResponse> HandleCommand(StartGameCommand request)
    {
        ...
        if (game is null)
        {
            return Result.Failure<StartGameResponse>(
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.BusinessRule("Game has already been started.")
            );
        }
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.BusinessRule("No host found for this game.")
            );
        }
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.Unauthorized("Only the host can start the game.")
            );
        }
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.BusinessRule("At least 2 players are required to start the game.")
            );
        }
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.BusinessRule("Failed to start game due to an unexpected error.")
            );
        }
        ...
        {
            return Result.Failure<StartGameResponse>(
                Error.BusinessRule("Game disappeared after start operation.")
            );
        }

        var response = new StartGameResponse(
            updatedGame.CurrentPlayer?.Nickname ?? "Unknown",
            updatedGame.CurrentCard?.Title
        );
